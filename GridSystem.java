...
//	}
	...
	public void movePlayer(int move) {
		if(isValidMovePlayer(move) ) {
			...
			// if moving off a goal position - change it back to a goal position when person steps off
			if(isGoalPosition(this.player.getPosition_x(), this.player.getPosition_y())) {
				modifyTile(this.player.getPosition_x(), this.player.getPosition_y(), '4');
			}	
	...
	private boolean isValidMovePlayer(int move) {
		...
		// Check if either empty space or a goal space
		
        //Up
		if((move == 1 && this.getCurrentState()[x-1][y] == '0') 
				|| (move == 1 && this.getCurrentState()[x-1][y] == '4')) {
			return true;
		}
		//Down
		if ((move == 2 && this.getCurrentState()[x+1][y] == '0') 
				|| (move == 2 && this.getCurrentState()[x+1][y] == '4')) {
			return true;
		}
		//Left
		if ((move == 3 && this.getCurrentState()[x][y-1] == '0')
				|| (move == 3 && this.getCurrentState()[x][y-1] == '4')) {
			return true;
		}
		//Right
		if ((move == 4 && this.getCurrentState()[x][y+1] == '0') 
				|| (move == 4 && this.getCurrentState()[x][y+1] == '4')) {
			return true;
		}
	...
	private boolean isValidMoveBox(Box b, int move) {
		...
		// move box to goal as well as empty space
	...
	// checks if given x,y coords are a goal position
	public boolean isGoalPosition(int x, int y) {
		if(this.initialBoard[x][y] == '4') {
			return true;
		}
		return false;
	}
